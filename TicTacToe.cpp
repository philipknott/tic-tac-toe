...
/**
 * Receives player input and returns a vector with row and column of chosen
 * cell.
...
int main() {
    ...
    for (int i = 0; i < 9; i++) {
        std::cout << std::endl;
        DisplayBoard(board);
        std::cout << "Player " << (player == 0 ? 'X' : 'O') << std::endl;
        std::vector<int> pos = GetPlayerChoice();
        PlaceMarker(board, pos, player % 2);
        player++;
    }
}